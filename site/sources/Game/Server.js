...
export class Server
{
    ...
    {
        this.local = false
        const endpoint = import.meta.env.VITE_SERVER_URL === 'auto' ? `${location.protocol === 'https:' ? 'wss' : 'ws'}://${location.host}/world` : import.meta.env.VITE_SERVER_URL
    ...
    send(message)
    {
        if(this.localWorld) return this.localWorld.send(message)
        if(!this.connected)
            return false
