    ...
    setServer()
    {
        ...
        const update = (connected) =>
        {
            if(connected)
            {
                ...
                text.textContent = this.game.server.local ? 'Local' : 'Online'

                tooltip.innerHTML = this.game.server.local ? '本机存档：留言、赛车榜和世界事件。可配置自建服务器共享。' : 'Connected to your own MoMoZi world server'
            }
            else
            {
                element.classList.remove('is-success')
