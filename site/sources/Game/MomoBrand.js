...
export function prepareMomoWorld(game) {
    ...
    const plaques = {x:'GH',bluesky:'XHS',youtube:'CODE',mail:'WX',twitch:'FILM',gitHub:'TY',linkedIn:'LAB',discord:'HF',onlyfans:'FANS'}
    ...
    const career = {
        careerFreelancerTexture: ['AGENT TPM', 'FOUNDATION MODEL / NOW'],
        careerHeticTexture: ['MEDICAL AI', 'ALGORITHM TO PRODUCT'],
        careerImmersiveGardenTexture: ['SENSETIME', 'MODEL TRAINING / AI TPM'],
        careerIRLTeacherTexture: ['PADDLEPADDLE', 'AI DEVELOPER PLATFORM'],
        careerOnlineTeacherTexture: ['BAIDU RESEARCH', 'AI PRODUCT'],
        careerUzikTexture: ['MEITUAN', 'SEARCH AGENT / AI PRODUCT']
    }
...
export function installMomoHud() {
    ...
    document.querySelectorAll('[data-momo-action]').forEach(button => button.addEventListener('click', () => {
        ...
        if (action === 'map') game.modals.open('map')
            ...
            game.menu.close(); game.modals.open('discord')
        }
