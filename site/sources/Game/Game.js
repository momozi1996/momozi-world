...
export class Game
{
    ...
    async init()
    {
        ...
        this.resources = await this.resourcesLoader.load([
            ...
            [ 'paletteTexture',             `momozi/palette.png${cb}`,                   'texture', (resource) => { resource.minFilter = THREE.NearestFilter; resource.magFilter = THREE.NearestFilter; resource.generateMipmaps = false; resource.colorSpace = THREE.SRGBColorSpace; } ],
        ...
        const resourcesPromise = this.resourcesLoader.load(
            [
                ...
                [ 'timeMachineScreenFolioTexture',         `momozi/momo-code-screen.png${cb}`,              'texture', (resource) => { resource.minFilter = THREE.NearestFilter; resource.magFilter = THREE.NearestFilter; resource.generateMipmaps = false; resource.wrapS = THREE.ClampToEdgeWrapping; resource.wrapT = THREE.ClampToEdgeWrapping; resource.colorSpace = THREE.SRGBColorSpace; } ],
            ],
            (toLoad, total) =>
            {
        ...
        prepareMomoWorld(this)
        this.terrain = new Terrain()
        prepareMomoNature(this)
