...
failed_paths={p for p,e in json.load(open('momozi-world/research/download-errors.json'))}; files=[x for x in tree if x['path'] in failed_paths]
...
json.dump(failed,open('momozi-world/research/download-errors.json','w'));print('DONE',len(failed),'errors',flush=True)
